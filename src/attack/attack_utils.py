...
def find_cross_model_cache(resources_dir: str, claim_id: int, min_num_fake: int, cache_attack_type: str,
                           current_model_suffix: str, attack_type: str, logger,
                           additional_dirs: list = None, legacy_cache_types: list = None):
    """
    Search for cached fake evidence from different model combinations,
    optionally across multiple directories and with legacy cache key formats.
    
    For justification-dependent attacks (fact2fiction, if2f), only reuse cache
    with the same fact-checker model but different attacker model.
    For other attacks, reuse cache from any model combination.
    
    Prefers exact count match; for larger counts, picks the smallest one.
    
    Args:
        resources_dir: Primary directory containing resource cache files
        claim_id: Claim ID to search for
        min_num_fake: Minimum number of fake evidence needed
        cache_attack_type: Attack type cache key (e.g. "if2f_hybrid_bm0.70")
        current_model_suffix: Current model cache suffix to exclude from results
        attack_type: Base attack type (for justification-dependency check)
        logger: Logger instance
        additional_dirs: Extra directories to search (e.g. sibling experiment dirs)
        legacy_cache_types: Alternative cache_attack_type strings for backward compat
        
    Returns:
        tuple: (found: bool, num_fake_in_cache: int, resources_path: str)
    """
    justification_dependent = ["fact2fiction", "if2f"]

    current_fc_model = None
    if attack_type in justification_dependent:
        if current_model_suffix == "":
            current_fc_model = "gpt-4o-mini"
        else:
            fc_match = re.match(r"_fc-(.+?)_att-(.+)", current_model_suffix)
            if fc_match:
                current_fc_model = fc_match.group(1)

    valid_cache_types = {cache_attack_type}
    if legacy_cache_types:
        valid_cache_types.update(legacy_cache_types)

    all_dirs = [resources_dir]
    if additional_dirs:
        all_dirs.extend(additional_dirs)

    prefix = f"{claim_id}_"
    best_exact = None
    best_larger_path = None
    best_larger_count = float('inf')
        ...
        try:
            for filename in os.listdir(search_dir):
                if not filename.startswith(prefix) or not filename.endswith(".pkl"):
                    continue
                if "_resources" not in filename:
                    continue

                try:
                    resources_idx = filename.index("_resources")
                except ValueError:
                    continue
                front_part = filename[len(prefix):resources_idx]
                model_suffix_part = filename[resources_idx + len("_resources"):-len(".pkl")]

                if search_dir == resources_dir and model_suffix_part == current_model_suffix:
                    continue

                try:
                    first_underscore = front_part.index("_")
                    file_num_fake = int(front_part[:first_underscore])
                    file_cache_type = front_part[first_underscore + 1:]
                except (ValueError, IndexError):
                    continue

                if file_cache_type not in valid_cache_types:
                    continue

                if attack_type in justification_dependent and current_fc_model:
                    if model_suffix_part == "":
                        cached_fc = "gpt-4o-mini"
                    else:
                        fc_match = re.match(r"_fc-(.+?)_att-(.+)", model_suffix_part)
                        if fc_match:
                            cached_fc = fc_match.group(1)
                        else:
                            continue
                    if cached_fc != current_fc_model:
                        continue

                resources_path = os.path.join(search_dir, filename)
                if file_num_fake == min_num_fake and best_exact is None:
                    best_exact = resources_path
                elif file_num_fake > min_num_fake and file_num_fake < best_larger_count:
                    best_larger_count = file_num_fake
                    best_larger_path = resources_path
        except Exception as e:
            logger.warning(f"Error searching for cross-model cache in {search_dir}: {e}")

    if best_exact:
        logger.info(f"Found cross-model exact-count cache: {best_exact}")
        return True, min_num_fake, best_exact
    elif best_larger_path:
        logger.info(f"Found cross-model larger cache ({best_larger_count} fake evidence): {best_larger_path}")
        return True, best_larger_count, best_larger_path
